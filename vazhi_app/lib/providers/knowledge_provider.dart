...
final thirukkuralProvider = FutureProvider.family<Thirukkural?, int>((
  ref,
  number,
) async {
...
final thirukkuralSearchProvider =
    FutureProvider.family<List<Thirukkural>, String>((ref, query) async {
...
final kuralsByAthikaramProvider = FutureProvider.family<List<Thirukkural>, int>(
  (ref, athikaramNumber) async {
...
final schemesByLevelProvider = FutureProvider.family<List<Scheme>, String>((
  ref,
  level,
) async {
...
final schemeByIdProvider = FutureProvider.family<Scheme?, String>((
  ref,
  id,
) async {
...
final nationalEmergencyContactsProvider =
...
final emergencyContactsByTypeProvider =
    FutureProvider.family<List<EmergencyContact>, String>((ref, type) async {
      final results = await KnowledgeDatabase.getEmergencyContacts(type: type);
      return results.map((m) => EmergencyContact.fromMap(m)).toList();
    });

/// Hospitals by district provider
final hospitalsByDistrictProvider =
    FutureProvider.family<List<Hospital>, String>((ref, district) async {
...
final hospitalSearchProvider = FutureProvider.family<List<Hospital>, String>((
  ref,
  query,
) async {
