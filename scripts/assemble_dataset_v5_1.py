#!/usr/bin/env python3
"""
Assemble Dataset v5.1 — Safety-rebalanced SFT dataset.

v5.1 changes from v5.0:
- Safety cut from 1,800 (30.6%) → ~200 (5%) to fix "தீங்கு" mode collapse
- Toxic_Matrix: 867 identical-response samples → 50 diverse questions
- HHRLHF_T: 933 → 150 response-deduplicated, topic-diverse samples
...
def select_diverse_safety(safety_items: list) -> list:
    ...
    # Shuffle deterministically, then pick every Nth to spread across topics
    random.seed(42)
    random.shuffle(toxic)
    # Deduplicate by instruction (some questions may repeat)
    seen_q = set()
    toxic_unique = []
    for item in toxic:
        q_hash = hashlib.md5(
    ...
    # Take evenly spaced samples for topic diversity
    ...
    # Group by response start (first 80 chars) to collapse near-duplicates
    response_groups = {}
    for item in hhrlhf:
        key = item["output"][:80]
    ...
    # From each response group, keep only 1 (the first)
...
def main():
    ...
    # 3. IndicAlign safety — REBALANCED for v5.1
    ...
    v50_total = sum(v50.values())
    for b, c in sorted(buckets.items(), key=lambda x: -x[1]):
        old = v50.get(b, 0)
        old_pct = old / v50_total * 100 if v50_total else 0
    ...
    train_path = OUTPUT_DIR / "vazhi-tamil-sft-v5_1-train.json"
    eval_path = OUTPUT_DIR / "vazhi-tamil-sft-v5_1-eval.json"
    full_path = OUTPUT_DIR / "vazhi-tamil-sft-v5_1-full.json"
    ...
    # Safety vocabulary check (the whole point of v5.1)
    print("\n" + "=" * 60)
    ...
    print("\n✅ Dataset v5.1 ready for upload to HuggingFace")
    print(
